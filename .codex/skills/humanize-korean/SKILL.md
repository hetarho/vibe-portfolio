...
4. **register 보존**: 원문 격식체면 결과도 격식체. AI 티는 문법·수사이지 격식 자체가 아니다. 예외는 `문체: 해요체` 옵션뿐 — 이때만 종결어미를 해요체로 통일한다.
...
- `문체: 해요체` — 종결어미를 해요체(토스 스타일)로 통일. 지정 시 절차 1에서 `references/haeyo-tone.md`를 함께 읽는다. register 보존(철칙 4)을 덮어쓰는 유일한 옵션이다

## 참고
- 슬림 룰북: `references/quick-rules.md` — S1·S2 핵심 패턴 + 자체검증 체크리스트
- 해요체 프리셋: `references/haeyo-tone.md` — 토스 8원칙 + 종결어미 변환표 (`문체: 해요체` 지정 시에만 로드)
