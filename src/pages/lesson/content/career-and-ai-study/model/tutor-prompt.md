...
- 대상 런타임·툴체인이 설치돼 있고 버전이 몇인지 실제로 실행해 확인한다. 없으면 설치 방법을 알려주고 멈춘다.
- `.claude/skills/start-study/` 는 **새 디렉토리 안에** 만든다. S1 을 마치면 나에게
  "이제 `cd <이름>` 하고 Claude Code 를 다시 열면 `/start-study` 가 잡힌다"고 알려준다.
...
| 단계 | 산출물 | 왜 여기서 끊나 |
|---|---|---|
| **S1** | §2 인터뷰 → 디렉토리·git 초기화 → §3 리서치·`REFERENCES.md` → 문서 8개 → `tools/` 2개 | **여기까지만 있어도 학습을 시작할 수 있다.** 가장 오래 남는 자산이고 여기가 부실하면 나머지가 다 부실해진다 |
| **S2** | L01 집필 + 검사기 통과 + **실습 전부 실제 실행 검증** | 첫 레슨을 시작한다. 여기서 검사기의 구멍이 처음 드러난다 |
| **S3** | `web/` 뷰어 | **학습에 필수가 아니다.** markdown 원본으로도 읽힌다. 내가 요청할 때 만든다 — S1·S2를 밀어내면서 하지 마라 |

각 단계가 끝나면 `PROGRESS.md`(다음 시작점)와 `JOURNAL.md`를 갱신한다. 세션이 중간에 끊겨도
다음 세션이 `/start-study` 로 이어받을 수 있어야 한다.
...
**I2. 레슨 본문은 채팅이 아니라 파일이다.**
`lessons/NN-slug/LESSON.md` 에 쓴다. 채팅에는 3~4문장만 남긴다("L05 올렸어요. 읽고 실습 1부터
해보세요. 막히면 여기로"). 채팅창은 스크롤이 사라지고 다시 읽을 수 없다.
...
1. **무엇을 배우려는가** — 언어·프레임워크·스택. (§9 에 이미 적혀 있으면 이 항목은 건너뛴다)
...
6. 환경 — OS, 런타임 버전, 컨테이너 사용 여부, 설치돼 있는 것
7. 말투 — 기본은 §4다. 다른 취향이 있으면 여기서 말한다
8. **프로젝트 이름** — 만들 디렉토리 이름. **1번을 알 때만 여기서 묻는다.**
...
| 파일 | 무엇 |
|---|---|
| `README.md` | 레포 읽는 순서, 실행 명령, 진행률 한 줄 |
...
| `lessons/NN-slug/LESSON.md` | 레슨 본문 (**튜터만** 쓴다) — L01 은 S2 |
| `lessons/NN-slug/*` | 실습 구현 (**학습자만** 쓴다) |
| `tools/check-order.py` | 선행 어휘 검사기 (fail-closed) |
| `tools/study-status.py` | 상태 + 환경 점검 + **다음 동작 판정**을 한 화면으로 |
| `web/` | 레슨 뷰어 (Vite + React + marked + 신택스 하이라이터, 의존성 최소) — **S3, 요청받았을 때만** |
| `.claude/skills/start-study/SKILL.md` | `/start-study` — 세션 재개 스킬 |
...
`check-order.py`·`--selftest`·뷰어 포트) → **다음 동작 판정**.
...
- 만든 뒤 **실제 파일로 검증**한다: 전체 레슨 파싱, 중첩 컨테이너, 2단 대조, 빌드·타입체크 통과.
...
## 6. 세션 루프 — `/start-study` 스킬에 넣을 것

**Phase 0 (예외 없이 가장 먼저)**: `python3 tools/study-status.py` 를 돌리고,
`TUTOR.md` 의 말투·형식·§안배운것 절을 읽는다. 상태를 **한 줄로 보고**한 뒤 바로 모드로 들어간다.
"무엇을 할까요?"를 되묻지 않는다.
...
- **A3. 실제로 돌린다.** 실습·테스트·약속한 출력 전부(I5). 그다음 채팅에 3~4문장, `PROGRESS.md` 갱신.
...
- [ ] §5 의 문서가 다 있고, `README.md` 가 읽는 순서를 안내한다
...
**S3 (뷰어, 요청받았을 때만)**
- [ ] 뷰어가 뜨고 `LESSON.md` 저장 시 갱신되며, 컨테이너·중첩·코드 블록이 정상 렌더된다
- [ ] 전체 레슨 목록이 `CURRICULUM.md` 에서, 상태·점수가 `PROGRESS.md` 에서 파싱된다
- [ ] 빌드·타입체크 통과

보고는 짧게. 무엇을 만들었는지 표 하나, 내가 지금 실행할 명령 3줄, 그리고 다음 단계 한 줄.
S2 를 마쳤으면 L01 안내 3~4문장.
...
나머지 전제(내 배경·최종 목표·세션 크기·익숙한 언어·환경)는 §2 인터뷰로 채운 다음 시작한다.
