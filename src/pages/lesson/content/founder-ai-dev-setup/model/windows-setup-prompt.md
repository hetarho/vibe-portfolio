...
이 PC에서 나중에 다음 작업을 시작할 수 있는 **기본 개발 환경만** 만들어라.
...
**오늘은 실제 프로젝트 실습을 하지 않는다.** 저장소를 clone하거나, 프로젝트 의존성을 설치하거나, 서비스를 실행·수정·테스트·배포하지 마라. 환경 도구의 설치와 버전·경로 검증까지만 끝내라.
...
6. 마지막에는 새 프로젝트를 시작할 기본 환경이 준비되었는지만 종합 점검해라.
...
현재 폴더에 프로젝트가 있더라도 파일을 읽거나 실행하지 말고, 환경 세팅만 진행해라.
...
프로젝트 실행·의존성 설치·테스트·빌드는 다음 미팅으로 남기고 오늘은 건드리지 마라.
...
4. 남은 문제: 없으면 `없음`
5. 다음 행동: 설정 보고서를 저장하고 오늘 작업을 끝내는 한 문장
